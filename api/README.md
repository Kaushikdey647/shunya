...
- **`/universes`** — CRUD on **`api_universes`**, membership under **`api_universe_members`** (equity-only when `symbol_classifications.quote_type` is present), **`GET /universes/{id}/tickers`** (flat member list), **`GET /universes/{id}/summary`** (sector/industry breakdown excluding unknown/other buckets; fundamentals aggregates from latest **`fundamentals_daily`** per member). Migration **`014_api_universes.sql`** adds tables and optional **`api_alphas.default_universe_id`**.
