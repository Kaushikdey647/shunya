...
| **`/instruments/...`** | Search, OHLCV (optional **`route`** query: `auto`, `best_effort`, or explicit upstream id; response includes **`provenance`** with **`read_path`** vs **`upstream_source_id`**), fundamentals-style panels; prefers Timescale when coverage is complete. **WebSocket** **`/instruments/{symbol}/stream/alpaca-l1`** — when Alpaca is enabled on the API, **IEX** L1 **quotes** (BBO) and **trades** as JSON (`hello` with `schema: 1` and `channels`, `quote`, `trade`, optional `trade_correction` / `trade_cancel`, `subscription` for Alpaca subscription acks, `error`). Outside **US listed equity regular hours** (weekdays **09:30–16:00** `America/New_York`, end-exclusive at 16:00; exchange holidays / early closes are **not** modeled), the server rejects the socket with **`error`** / **`code: us_rth_closed`** and also emits a **`publish_notification`** on **`/notifications/stream`** (same human-readable message; **`context.symbol`** is the requested ticker) unless **`SHUNYA_ALPACA_L1_IGNORE_US_RTH`** is set for dev. All browser sessions share **one** Alpaca market-data WebSocket per API key in the API process (see **`api.services.alpaca_l1_feed_hub`**); distinct symbols are capped by **`SHUNYA_ALPACA_L1_MAX_SYMBOLS`** (default **30**); over-cap returns **`error`** with **`code: symbol_limit`**. If the shared Alpaca stream stops (for example **connection limit exceeded**), clients receive **`error`** with **`code: alpaca_market_data_stopped`**. When Alpaca sends a **control-stream** error (previously log-only in `alpaca-py`), clients receive **`error`** with **`code: alpaca_upstream`** and a short message. Creating the **next** shared stream waits **`SHUNYA_ALPACA_L1_RECONNECT_COOLDOWN_SEC`** (default **5** seconds; see **`.env.example`**) so Alpaca can release the prior session. Stocks/ETFs only. The legacy **`/instruments/{symbol}/stream/alpaca-bars`** endpoint returns **`deprecated_stream`** and closes. |
| **`/notifications/stream`** | **WebSocket** — in-process notification fan-out for connected browsers. First frame: **`{ "type": "hello", "schema": 1 }`**. Subsequent **`{ "type": "notification", "schema": 1, "id", "ts", "level": "error" \| "warning" \| "info", "message", ... }`** (optional **`code`**, **`title`**, **`context`**). Typical **`code`** values include **`backtest.queued`**, **`backtest.started`**, **`backtest.succeeded`**, **`backtest.deleted`** (and batch-delete), worker **`backtest.failed`**; **`alpha.created`** / **`alpha.updated`** / **`alpha.deleted`**; **`universe.created`** / **`universe.updated`** / **`universe.deleted`** / **`universe.members_*`**; **`settings.app_patched`** after **`PATCH /settings/app`**; **`us_rth_closed`** from the Alpaca L1 WebSocket path. **Multi-worker / multi-replica:** subscribers are per API process only; use a single worker for consistent delivery in dev or add a broker later for cross-replica fan-out. |
...
| **`GET /settings/market-clock`** | Server-side market clock snapshot: formatted **US** and **India** listing-time lines, **`us_listed_rth_open`**, and **`alpaca_l1_us_equities_stream_allowed`** (US regular session gate for the Alpaca L1 WebSocket; overridable with **`SHUNYA_ALPACA_L1_IGNORE_US_RTH`** for local dev). |
| **WebSocket `/settings/market-clock/stream`** | Live clock: **`hello`** (`schema: 1`) then **`tick`** frames with the same fields as GET. The API process runs one broadcaster loop (interval **`SHUNYA_MARKET_CLOCK_TICK_SECONDS`**, default **1**, clamped **0.25–60**). In-process subscribers use **`api.services.market_clock_hub.create_market_clock_subscription`** / **`release_market_clock_subscription`**. |
...
| `SHUNYA_ALPACA_L1_RECONNECT_COOLDOWN_SEC` | Seconds to wait after Alpaca L1 teardown before the API opens the next shared market-data stream (default **5**; reduces **connection limit exceeded** on rapid reconnect). |
| `SHUNYA_ALPACA_L1_IGNORE_US_RTH` | When truthy, **do not** gate **`/instruments/.../stream/alpaca-l1`** on US regular session (dev only; production should leave unset). |

**L1 WebSocket diagnostics:** from the repo root, run **`uv run python scripts/diag_alpaca_l1_ws.py http://127.0.0.1:8000 AAPL 30`** to print JSON frames (`hello`, `quote`, `subscription`, `error`, …) without the browser.
